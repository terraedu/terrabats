...
public class TeleopBlue extends CommandOpMode {
    ...
    public void initialize() {
        ...
        new Trigger(() -> gph1.getTrigger(GamepadKeys.Trigger.RIGHT_TRIGGER) > 0.1 && deposit == PlaceMode.SPECIMEN).whenActive(
                new ConditionalCommand(
                        toSpecimenMode(),
        ...
        new Trigger(() -> gph1.getTrigger(GamepadKeys.Trigger.RIGHT_TRIGGER) > 0.1 && deposit == PlaceMode.SAMPLE).whenActive(
                new ConditionalCommand(
                        sampleTransfer(),
                        samplePlace(),
                        () -> status == RobotMode.DRIVING
        ...
        gph1.getGamepadButton(GamepadKeys.Button.RIGHT_STICK_BUTTON).whenPressed(toggleSpecimenLinkage());

        // Right Trigger – Intake Logic (Auto-Intake If Driving)
        new Trigger(() -> gph1.getTrigger(GamepadKeys.Trigger.LEFT_TRIGGER) > 0.8).whenActive(
                new ConditionalCommand(
                        startAutoIntake(),
                        manualEject(),
                        () -> status == RobotMode.DRIVING
